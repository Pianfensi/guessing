...
class MusicBrainz(API):
	def __init__(self):
		self._categories = {
			"albums" : {
				...
				"questionTypes" : {
					"tracks" : self.__getAlbumTracks,
					"duration" : self.__getAlbumDuration,
					"releaseyear" : self.__getAlbumRelease
				}
			},
			"artists" : {
				"method" : self.__getArtist,
				"questionTypes" : {
					"founded" : self.__getGroupFounded,
					"ended" : self.__getGroupEnded,
					"birthyear" : self.__getArtistBirthyear,
					"deathyear" : self.__getArtistDeathyear
				}
			}
		}
	...
	def __getAlbum(self):
		...
		try:
			results = musicbrainzngs.search_releases(query="", limit=10, offset=None, strict=False, country=random_country, mediums="CD", type="Album")
			offset = random.randint(0, results["release-count"]-100)
			result = musicbrainzngs.search_releases(query="", limit=100, offset=offset, strict=False, country=random_country, mediums="CD", type="Album", reid="1ec09df8-aef8-48f1-91a9-92a982bf0dfe")
		except:
			return 0
	...
	def __getArtist(self):
		random_country = random.choice(self.__country_field)
		musicbrainzngs.set_useragent(
			"guessing-game-pianfensi",
			"1.a",
			CONTACT
		)
		try:
			results = musicbrainzngs.search_artists(query="", limit=10, offset=None, strict=False, country=random_country)
			offset = random.randint(0, results["artist-count"]-100)
			result = musicbrainzngs.search_artists(query="", limit=100, offset=offset, strict=False, country=random_country)
			self._success = True
		except:
			return 0
